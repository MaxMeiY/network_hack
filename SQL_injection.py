...
def get_abs_url(link):
    '''
    check if the link is relative and prepend the
    protocol and host. filter unwanted links like mailto
    and links that do not go to our base host
    '''
    if link:
        if "://" not in link:
            if link[0] != "/":
...
def spider(url):
    '''
    check if we don't known the url
    spider to url
    extract new links
...
def attack(url):
    ...
    if not a_query in already_attacked.get(a_path, []):
        ...
        try:
            ...
            for param_value in a_query.splite("&"):
                param, value = param_value.splite("=")
                ...
                for inject in inject_chars:
                    ...
                    sys.stdout.write(".")
                    sys.stdout.flush()
                    a_res, a_content = browser.request(
                        a_url
                    )
...
(protocol, base_host, path,
 params, query, frag) = urlparse(start_url)
