...
# Patterns that match username and passwords
pattern = re.compile(r"""(?P<found>(USER|USERNAME|PASS|
                        PASSWORD|LOGIN|BENUTZER|PASSWORT|
                        AUTH|ACCESS|ACCESS_?KEY|SESSION|
                       SESSION_?KEY|TOKEN)[=:\s].+)\b""",
                     re.MULTILINE|re.IGNORECASE)

# This function will be called for every packet, decode
# it and try to find a username or passwords in it
...
def handler_packet(hdr, data):
    ...
    if not tcp_pkt.get_SYN() and not tcp_pkt.get_RST() \
       and not tcp_pkt.get_FIN() and match and \
